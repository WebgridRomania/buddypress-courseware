...
class BPSP_Responses {
    ...
    function single_response_screen( $vars ) {
        ...
        if(
            ...
            $vars['die'] = __(
					'BuddyPress Courseware Error while forbidden user tried to access a private response.',
					'bpsp'
					);
            return $vars;
        }
        ...
        if(  is_super_admin() || $this->has_response_caps() ) {
            $vars['response_delete_permalink'] =
					$vars['assignment_permalink'] .
					'/response/' .
					$response->post_name .
					'/delete';
            $vars['response_delete_uri'] = add_query_arg(
					'_wpnonce',
					wp_create_nonce( $nonce_delete_name ),
					$vars['response_delete_permalink']
					);
        }
