    ...
    function import_gradebook_screen( $vars ) {
        ...
        if( isset( $_FILES['csv_filename'] ) && !empty( $_FILES['csv_filename'] )) {
            ...
            if( count( $csv->data ) == count( $grades ) ) {
                $vars['message'] = __(
						'Data imported successfully, but it is not saved yet! Save this form changes to keep the data.',
						'bpsp'
						);
			} else {
                $vars['error'] = __(
						'File data contains error or entries from other gradebook. Please check again.',
						'bpsp'
						);
			}
        }
