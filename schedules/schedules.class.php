...
class BPSP_Schedules {
    ...
    function datecheck( $starttime, $endtime, $repetition = null ){
        ...
        if( is_array( $repetition ) ) {
            if(
					!in_array( $repetition['interval'], array( 'day', 'week', 'month' )) ||
    ...
    function new_schedule_screen( $vars ) {
        ...
        if( isset( $_POST['schedule'] ) && $_POST['schedule']['object'] == 'group' && isset( $_POST['_wpnonce'] ) ) {
            if(
					empty( $_POST['schedule']['title'] ) ||
					empty( $_POST['schedule']['desc'] ) ||
					empty( $_POST['schedule']['start_date'] )
					) {
    ...
    function edit_schedule_screen( $vars ) {
        ...
        if(
				!$this->has_schedule_caps( $bp->loggedin_user->id ) ||
				!is_super_admin() &&
				$bp->groups->current_group->id != $old_schedule->terms[0]->name
				) {
            $vars['die'] = __(
					'BuddyPress Courseware Error while forbidden user tried to update the schedule.',
					'bpsp'
					);
            ...
            if( true != $is_nonce ) {
                ...
                if( !empty( $updated_schedule['group_id'] ) && $valid_dates ) {
                    ...
                    if( $updated_schedule_id ) {
                        if(
								!empty( $updated_schedule['course_id'] ) &&
								BPSP_Courses::is_course( $updated_schedule['course_id'] )
								) {
                        ...
                        update_post_meta(
								$updated_schedule_id,
								'start_date',
								$updated_schedule['start_date'],
								$old_schedule->start_date
								);
                        update_post_meta(
								$updated_schedule_id,
								'end_date',
								$updated_schedule['end_date'],
								$old_schedule->end_date
								);
        ...
        $vars['nonce'] = wp_nonce_field( $nonce_name, '_wpnonce', true, false );
        $vars['delete_nonce'] = add_query_arg(
				'_wpnonce',
				wp_create_nonce( 'delete_schedule' ),
				$vars['schedule_delete_uri']
				);
        $vars['trail'] = array(
